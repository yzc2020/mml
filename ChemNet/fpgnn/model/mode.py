...
class GATConv(nn.Module):
    def __init__(
        ...
        tag = False
        if att == "total":
            self.bond_src = nn.Linear( self.in_feats, self.elength * num_heads, bias=tag)
            self.bond_dst = nn.Linear( self.in_feats, self.elength * num_heads, bias=tag)
...
class DMPNN(nn.Module):
    def __init__(self,in_features,out_features,level):
    ...
    def forward(self,dist,dist3d,feat):
        e = self.attfc(feat)
        
        dist[dist!=self.level] = 0
        dist[np.diag_indices_from(dist)] = 1

        dist3d[np.diag_indices_from(dist3d)]=1
        dist3d = 1/(dist3d*dist3d)
        dist3d = dist*dist3d
        cooadj = scipy.sparse.coo_matrix(dist)
        graph = dgl.from_scipy(distweight,eweight_name="dist")
        ...
        with graph.local_scope():
            ...
            #print(graph.ndata['feat_src'])
            #graph.edata.update({"weight":weight})
            graph.edata["a"] = edge_softmax(graph, weight)
...
class GNN(nn.Module):
    def __init__(self,args):
        ...
        self.att = SelfAttention(256,4,dropout_prob = 0.2)
        ...
        self.dmpnn3 = DMPNN(self.atom_feat_num*self.nheads,
                   self.atom_feat_num*self.nheads,
                   3)

        self.dmpnn4 = DMPNN(self.atom_feat_num*self.nheads,
                   self.atom_feat_num*self.nheads,
                   4)
        self.dmpnn5 = DMPNN(self.atom_feat_num*self.nheads,
                   self.atom_feat_num*self.nheads,
                   5)
    ...
    def forward(self,smiles):
        ...
        for i,one in enumerate(smiles):
            ...
            if one in self.smile2graph:
                #cooadj = self.smile2graph[one]
                distweight = self.smile2dist[one]
                #edge_feat = self.smile2edge[one]
...
def FPGNN(args):
    ...
    return model
        ...
        self.edge_fc1 = nn.Linear(self.edge_feats,self.out_feats,bias=True)
        self.edge_fc2 = nn.Linear(self.out_feats,self.num_heads*self.out_feats,bias=True)
            #edge_feat = self.edge_fc1(bond_feat)
            #edge_feat = nn.functional.elu(edge_feat)
            #edge_feat = self.edge_fc2(edge_feat).view(-1,self.num_heads,self.out_feats)