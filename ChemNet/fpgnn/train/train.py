...
def epoch_train(model,data,loss_f,optimizer,scheduler,args,epoch):
    t1 = time.time()
    model.train()
    data.random_data(args.seed+epoch)
    loss_sum = 0
    data_used = 0
...
def fold_train(args, log):
    ...
    for epoch in range(args.epochs):
        ...
        epoch_train(model,train_data,loss_f,optimizer,scheduler,args,epoch)
        ...
        info(f'Test {args.metric} = {ave_test_score:.6f}')
        ...
        if data_type == 'classification' and ave_test_score >bestp:
            bestep=epoch
            bestp=ave_test_score
        if data_type == 'regression' and ave_test_score <bestp:
            bestep=epoch
            bestp=ave_test_score 

    
    print("bad counter",bad_counter)
    print("best bad counter",best_bad_counter)
    info(f'Best validation {args.metric} = {best_score:.6f} on epoch {best_epoch}')
    info(f'Best Test {args.metric} = {bestp:.6f} on epoch {bestep}')
